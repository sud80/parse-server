...
var constructKeys = function constructKeys(keys) {
  if (!_.isString(keys)) {
    return _.isEmpty(keys) ? undefined : keys;
  }
  var parsedKeys = {};
  if (!_.isEmpty(keys)) {
    _.forEach(keys.split(','), function (key) {
      var parts = key.split('.');
      var top = parts.splice(0, 1);
      var remaining = parts.join('.');
...
RestQuery.prototype.handleInclude = function () {
  ...
  var pathResponse = includePath(this.config, this.auth, this.response, this.include[0], this.keys);
...
function includePath(config, auth, response, path, keys) {
  ...
  var queryPromises = Object.keys(pointersHash).map(function (className) {
    ...
    var query = new RestQuery(config, auth, className, where, { keys: keys });
