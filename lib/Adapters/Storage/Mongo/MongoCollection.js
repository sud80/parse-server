...
var MongoCollection = function () {
  ...
  _createClass(MongoCollection, [{
    ...
    value: function find(query) {
      ...
      return this._rawFind(query, { skip: skip, limit: limit, sort: sort, fields: fields, hint: hint }).catch(function (error) {
        // Check for "no geoindex" error
        if (error.code != 17007 && !error.message.match(/unable to find index for .geoNear/)) {
          throw error;
        }
        // Figure out what key needs an index
        var key = error.message.match(/field=([A-Za-z_0-9]+) /);
        key = key && key[1];
        if (!key) {
          throw error;
        }
        ...
        // Retry, but just once.
        .then(function () {
          return _this._rawFind(query, { skip: skip, limit: limit, sort: sort, fields: fields, hint: hint });
        });
      });
    }
  }, {
    ...
    value: function _rawFind(query) {
      ...
      return this._mongoCollection.find(query, { skip: skip, limit: limit, sort: sort, fields: fields, hint: hint }).toArray();
    }
  }, {
    key: 'count',
