...
RestWrite.prototype.execute = function () {
  ...
  return Promise.resolve().then(function () {
    return _this.checkMasterOnlyWriteFields();
  }).then(function () {
    return _this.checkImmutableFields();
  }).then(function () {
...
RestWrite.prototype.checkMasterOnlyWriteFields = function () {
  ...
  var masterOnlyWriteFields = this.classConfig.masterOnlyWriteFields;
  if (masterOnlyWriteFields && masterOnlyWriteFields.size > 0) {
    var field = void 0;
    if (_lodash2.default.some(this.data, function (v, k) {
      field = k;
      return masterOnlyWriteFields.has(k);
    })) {
      throw new Parse.Error(Parse.Error.OPERATION_FORBIDDEN, 'This user is not allowed to write ' + this.className + ' ' + (this.query && this.query.objectId) + ' ' + field);
    }
  }
};

RestWrite.prototype.checkImmutableFields = function () {
  var immutableFields = this.classConfig.immutableFields;
  ...
  if (_lodash2.default.some(this.data, function (v, k) {
    ...
    throw new Parse.Error(Parse.Error.OPERATION_FORBIDDEN, 'Cannot modify ' + field + ' of ' + this.className + ' ' + this.query.objectId);
  }
};
...
RestWrite.prototype.runBeforeTrigger = function () {
  ...
  var updatedObject = Parse.Object.fromJSON(originalData);
  ...
  return Promise.resolve().then(function () {
    return triggers.maybeRunTrigger(triggers.Types.beforeSave, _this4.auth, updatedObject, originalObject, _this4.config, _this4.triggerState);
...
RestWrite.prototype.handleInstallation = function () {
  ...
  if (_lodash2.default.isString(this.data.deviceToken)) {
...
RestWrite.prototype.runAfterTrigger = function () {
  ...
  var hasLiveQuery = this.config.liveQueryController && this.config.liveQueryController.hasLiveQuery(this.className);
